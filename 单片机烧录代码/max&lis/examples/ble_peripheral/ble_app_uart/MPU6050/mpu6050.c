...
void twi_handler(nrf_drv_twi_evt_t const * p_event, void * p_context)
{
	  ...
	  switch (p_event->type)
    {
        //传输完成事件
			...
			//MY_LOG_DEBUG("mpu initial sucess!");
            break;
...
bool mpu6050_register_read(uint8_t register_address, uint8_t * destination, uint8_t number_of_bytes)
{
	  ...
	  //MY_LOG_DEBUG("ERR_CODE = 0X%X",err_code);
	  //等待TWI总线传输完成
		while (m_xfer_done == false){}
...
//		 }
//	MY_LOG_DEBUG("%s", Gyrol_output);
//	ble_send(Gyrol_output,sizeof(Gyrol_output));
    ...
    {
        // snprintf ????????????
        send_len = snprintf(Gyrol_output, sizeof(Gyrol_output),
                            "x=%d y=%d z=%d",
                            GyroValue[0],
                            GyroValue[1],
                            GyroValue[2]);
    ...
    {
        send_len = snprintf(Gyrol_output, sizeof(Gyrol_output), "Read Gyro failed");
    }
   ...
   // MY_LOG_DEBUG("%s", Gyrol_output);
    ...
    if(send_len > 0)
    {
        ble_send((uint8_t *)Gyrol_output, send_len); 
    }	 
